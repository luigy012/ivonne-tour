...
const IC = {
  tierra: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2"/><path d="M15.5 8.5l-2 5-5 2 2-5 5-2z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>',
  agua: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 3.5s6 5.6 6 9.5a6 6 0 01-12 0c0-3.9 6-9.5 6-9.5z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>',
  eventos: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 3l1.9 4.6L18.5 9l-4.6 1.4L12 15l-1.9-4.6L5.5 9l4.6-1.4L12 3z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>'
};
const CATS = [{
    accent: '#1a9e5c',
    deep: '#0c5a34',
    glow: 'rgba(26,158,92,.5)',
    icon: IC.tierra,
    hero: PIC('ivonnetierra9', 1600, 1000),
    badge: 'Aventura en Tierra',
    headline: 'Desafía tus <span class="hl">Límites</span> en el Trópico',
    sub: 'Buggies, caballos y senderos escondidos para los que buscan pura adrenalina en la isla.',
    cta: 'Explorar Aventura',
    items: [{
      t: 'Buggy Aventura',
      s: 'Adrenalina',
      img: PIC('buggy4', 300, 300)
    }, {
      t: 'Cabalgata en la Playa',
      s: 'Relax',
      img: PIC('horse7', 300, 300)
    }, {
      t: 'Safari en Camión',
      s: 'Cultura',
      img: PIC('safari2', 300, 300)
    }]
  },
  {
    accent: '#0e93c9',
    deep: '#075a86',
    glow: 'rgba(14,147,201,.5)',
    icon: IC.agua,
    hero: PIC('ivonneagua5', 1600, 1000),
    badge: 'El Caribe te Espera',
    headline: 'Vive el <span class="hl">Mar</span> como Nunca Antes',
    sub: 'Snorkel, catamarán y jet ski en las aguas más claras del Caribe dominicano.',
    cta: 'Descubre el Mar',
    items: [{
      t: 'Snorkel en Arrecife',
      s: 'Naturaleza',
      img: PIC('snorkel3', 300, 300)
    }, {
      t: 'Catamarán Party',
      s: 'Fiesta',
      img: PIC('catamaran8', 300, 300)
    }, {
      t: 'Jet Ski por la Costa',
      s: 'Velocidad',
      img: PIC('jetski6', 300, 300)
    }]
  },
  {
    accent: '#ff7a2f',
    deep: '#b0430f',
    glow: 'rgba(255,122,47,.5)',
    icon: IC.eventos,
    hero: PIC('ivonneeventos7', 1600, 1000),
    badge: 'Momentos Únicos',
    headline: '<span class="hl">Noches</span> que No se Olvidan',
    sub: 'Cenas bajo las estrellas, fiestas y shows que le ponen el broche de oro a tu viaje.',
    cta: 'Planea tu Evento',
    items: [{
      t: 'Cena Bajo las Estrellas',
      s: 'Romántico',
      img: PIC('dinner5', 300, 300)
    }, {
      t: 'Fiesta en Catamarán',
      s: 'Fiesta',
      img: PIC('party9', 300, 300)
    }, {
      t: 'Show Cultural',
      s: 'Cultura',
      img: PIC('show1', 300, 300)
    }]
  }
];
...
heroImgs.forEach((el, i) => el.style.backgroundImage = `url("${CATS[i].hero}")`);
CATS.forEach((_, i) => {
...
function renderDeck(c) {
  ...
  c.items.forEach(it => {
    ...
    b.className = 'box';
    b.innerHTML = `<div class="th"><img src="${it.img}" alt=""></div><div class="tx"><div class="t">${it.t}</div><div class="s">${it.s}</div></div><span class="go"><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M9 5l7 7-7 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>`;
    deck.appendChild(b);
  });
}
...
function paint(c) {
  ...
  stage.style.setProperty('--accent-deep', c.deep);
  stage.style.setProperty('--glow', c.glow);
  badge.textContent = c.badge;
  headline.innerHTML = c.headline;
  sub.textContent = c.sub;
  ctaTxt.textContent = c.cta;
  ctaIcon.innerHTML = c.icon;
  heroImgs.forEach((el, i) => el.classList.toggle('on', i === idx));
...
function go(i, manual) {
  ...
  const c = CATS[idx];
  deck.classList.add('swap');
  setTimeout(() => {
    renderDeck(c);
    deck.classList.remove('swap');
  }, 280);
  paint(c);
  if (manual) restart();
}
...
document.getElementById('hero').addEventListener('mouseleave', restart);
renderDeck(CATS[0]);
paint(CATS[0]);
restart();
